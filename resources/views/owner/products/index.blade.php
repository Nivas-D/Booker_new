            ...
            <div class="header-body">
                <div class="row align-items-center py-4">
                    ...
                    <div class="col-lg-6 col-5 text-right">
                        <a href="{{ route('owner.products.create') }}" class="btn btn-sm btn-neutral">Create New Product</a>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <tbody>
                                    @foreach ($products as $product)
                                        <tr>
                                            ...
                                            <td class="text-primary">{{ ucwords($product->active) }}</td>
                                            <td class="text-left">
                                                <span class="float-right">
                                                    <a class="btn btn-sm btn-neutral" href="{{ route('owner.products.show', $product) }}">
                                                        <i class="fas fa-eye"></i> View
                                                    </a>
                                                    <a class="btn btn-sm btn-neutral" href="{{ route('owner.products.edit', $product) }}">
                                                        <i class="fas fa-info"></i> Edit
                                                    </a>
                                                    <a class="btn btn-sm btn-neutral" href="#" onclick="deleteProduct()">
                                                        <i class="fas fa-eraser"></i> Delete
                                                    </a>
                                                    <form action="{{ route('owner.products.destroy', $product) }}" method="POST" id="deleteProduct">
                                                        @method('DELETE')
                                                        @csrf
                                                    </form>
                                                </span>
