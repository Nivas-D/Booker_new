            ...
            <div class="header-body">
                <div class="row align-items-center py-4">
                    ...
                    <div class="col-lg-6 col-5 text-right">
                        <a href="{{ route('owner.services.create') }}" class="btn btn-sm btn-neutral">Create New Service</a>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <tbody>
                                    @foreach ($services as $service)
                                        <tr>
                                            ...
                                            <td class="text-primary">{{ ucwords($service->price_discounted) }} CHF</td>
                                            <td class="text-primary">{{ $service->industry_name }}</td>
                                            <td class="text-primary">{{ ucwords($service->active) }}</td>
                                            <td class="text-left">
                                                <span class="float-right">
                                                    <a class="btn btn-sm btn-neutral" href="{{ route('owner.services.show', $service) }}">
                                                        <i class="fas fa-eye"></i> View
                                                    </a>
                                                    <a class="btn btn-sm btn-neutral" href="{{ route('owner.services.edit', $service) }}">
                                                        <i class="fas fa-info"></i> Edit
                                                    </a>
                                                    <a class="btn btn-sm btn-neutral" href="#" onclick="deleteService()">
                                                        <i class="fas fa-eraser"></i> Delete
                                                    </a>
                                                    <form action="{{ route('owner.services.destroy', $service) }}" method="POST" id="deleteService">
                                                        @method('DELETE')
                                                        @csrf
                                                    </form>
                                                </span>
