...
class BusinessController extends Controller {
    ...
    public function index(Request $request){
        ...
        if ($request->has('searchBusiness') && $request->input('searchBusiness')!== '') {
            ...
            $business = DB::table('business')->where('business.name', 'LIKE',"%{$searchText}%")->orWhere('business.email', 'LIKE',"%{$searchText}%")->orWhere('business.telephone', 'LIKE',"%{$searchText}%")->orWhere('business.company_name', 'LIKE',"%{$searchText}%")->orWhere('categories.name', 'LIKE',"%{$searchText}%")->orWhere('business.created_at', 'LIKE',"%{$searchText}%")->orderby($orderByValue,$orderBy)
